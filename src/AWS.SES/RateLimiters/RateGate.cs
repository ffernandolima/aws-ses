...
namespace AWS.SES.RateLimiters
{
    ...
    internal class RateGate : IDisposable
    {
        ...
        public RateGate(int occurrences, TimeSpan timeUnit)
        {
            ...
            if (occurrences <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(occurrences), $"{nameof(occurrences)} must be a positive integer.");
            }

            if (timeUnit != timeUnit.Duration())
            {
                throw new ArgumentOutOfRangeException(nameof(timeUnit), $"{nameof(timeUnit)} must be a positive span of time.");
            }

            if (timeUnit >= TimeSpan.FromMilliseconds(uint.MaxValue))
            {
                throw new ArgumentOutOfRangeException(nameof(timeUnit), $"{nameof(timeUnit)} must be less than 2^32 milliseconds.");
            }
        ...
        private void ExitTimerCallback(object state)
        {
            ...
            if (exitTimeValid)
            {
                timeUntilNextCheck = Math.Min(TimeUnitMilliseconds, Math.Max(0, unchecked(exitTime - Environment.TickCount)));
            }
            else
            {
        ...
        /// <returns>true if the thread is allowed to proceed, or false if timed out</returns>
        public bool WaitToProceed(TimeSpan timeout) => WaitToProceed((int)timeout.TotalMilliseconds);

        /// <summary>
        /// Blocks the current thread indefinitely until allowed to proceed.
        /// </summary>
        public void WaitToProceed() => WaitToProceed(Timeout.Infinite);
